<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>我的獎勵計畫表 + 兌換商店</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;500;700&display=swap');
        
        body { font-family: 'Noto Sans TC', sans-serif; background-color: #f0fdfa; }

        /* 隱藏直向捲軸但保持功能 */
        .no-scrollbar::-webkit-scrollbar { display: none; }
        .no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }

        .task-checkbox:checked + div { text-decoration: line-through; color: #9ca3af; }
        
        /* 月曆格子樣式 */
        .calendar-day { aspect-ratio: 1; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding-top: 4px; border-radius: 8px; cursor: pointer; transition: all 0.2s; font-size: 0.9rem; position: relative; }
        .calendar-day:hover { background-color: #e0e7ff; }
        .calendar-day.active { background-color: #4f46e5; color: white; }
        .calendar-day.today { border: 2px solid #4f46e5; font-weight: bold; }
        
        /* 任務點點 (粉紅) */
        .calendar-day.has-task::after { content: ''; width: 4px; height: 4px; background-color: #ec4899; border-radius: 50%; margin-top: 2px; }
        .calendar-day.active.has-task::after { background-color: white; }

        /* 假日點點 (紅色) */
        .calendar-day.is-holiday::before { 
            content: ''; position: absolute; top: 4px; right: 4px; 
            width: 4px; height: 4px; background-color: #ef4444; border-radius: 50%; 
        }
        .calendar-day.active.is-holiday::before { background-color: #fca5a5; }

        /* 切換按鈕動畫 */
        .tab-btn { transition: all 0.3s; border-bottom: 3px solid transparent; opacity: 0.6; }
        .tab-btn.active { border-bottom-color: white; opacity: 1; font-weight: bold; }

        /* 視圖切換動畫 */
        .view-container { transition: transform 0.3s ease-in-out, opacity 0.3s; }
    </style>
</head>
<body class="flex flex-col items-center justify-center min-h-[100dvh] text-gray-800 p-2 md:p-4 box-border">

    <div class="w-full max-w-md bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden flex flex-col h-[calc(100dvh-1rem)] md:h-auto md:min-h-[750px] relative">
        
        <!-- Header & Tabs -->
        <div class="bg-indigo-600 p-4 md:p-6 pb-2 text-white flex flex-col shadow-md z-10 shrink-0">
            <div class="flex justify-between items-start mb-3">
                <div>
                    <h1 class="text-xs md:text-sm opacity-80 font-medium">獎勵計畫表</h1>
                    <div class="flex items-baseline gap-2">
                        <div id="header-date" class="text-xl md:text-2xl font-bold tracking-wide">載入中...</div>
                        <!-- Header 顯示今日節日 (如果有) -->
                        <div id="header-holiday" class="text-xs bg-red-500 text-white px-2 py-0.5 rounded-full hidden"></div>
                    </div>
                </div>
                <div class="bg-indigo-500/50 rounded-lg p-2 px-3 text-center backdrop-blur-sm min-w-[70px]">
                    <div class="text-[10px] md:text-xs opacity-80">持有金幣</div>
                    <div class="text-lg md:text-xl font-bold text-yellow-300">
                        $<span id="current-balance">0</span>
                    </div>
                </div>
            </div>

            <!-- Tabs -->
            <div class="flex gap-2 mt-1">
                <button onclick="switchTab('daily')" id="tab-daily" class="tab-btn active pb-2 flex-1 text-center text-sm">今日計畫</button>
                <button onclick="switchTab('monthly')" id="tab-monthly" class="tab-btn pb-2 flex-1 text-center text-sm">每月計畫</button>
                <button onclick="switchTab('shop')" id="tab-shop" class="tab-btn pb-2 flex-1 text-center text-sm">兌換商店</button>
            </div>
        </div>

        <!-- Main Content Area -->
        <div class="flex-1 overflow-hidden relative bg-gray-50">
            
            <!-- VIEW: Daily / Specific Date List -->
            <div id="view-list" class="view-container absolute inset-0 flex flex-col bg-gray-50 z-10">
                
                <!-- 假日橫幅提醒 -->
                <div id="holiday-alert-banner" class="hidden bg-red-100 text-red-600 px-4 py-2 text-sm font-bold text-center flex items-center justify-center gap-2 shrink-0 border-b border-red-200">
                    <i class="fas fa-gift"></i> <span id="holiday-name"></span>
                </div>

                <!-- Date Indicator -->
                <div id="selected-date-banner" class="hidden bg-yellow-100 text-yellow-800 text-center py-1 text-sm font-medium flex justify-between px-4 items-center shrink-0">
                    <span class="truncate text-xs"><i class="fas fa-calendar-alt mr-1"></i> <span id="editing-date-display"></span></span>
                    <button onclick="backToToday()" class="text-indigo-600 hover:text-indigo-800 underline text-xs whitespace-nowrap ml-2">回到今日</button>
                </div>

                <!-- Input Area -->
                <div class="p-3 md:p-4 bg-white border-b border-gray-100 shadow-sm z-10 shrink-0">
                    <div class="flex gap-2 mb-2">
                        <input type="text" id="task-input" 
                            class="flex-[2] bg-gray-50 border border-gray-200 text-gray-900 rounded-lg md:rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none p-2 md:p-3 text-sm md:text-base" 
                            placeholder="計畫要做什麼？">
                        <input type="number" id="reward-input" 
                            class="w-20 md:flex-[1] bg-gray-50 border border-gray-200 text-gray-900 rounded-lg md:rounded-xl focus:ring-2 focus:ring-yellow-400 outline-none p-2 md:p-3 text-center text-sm md:text-base" 
                            placeholder="$">
                    </div>
                    <button onclick="addTask()" 
                        class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg md:rounded-xl py-2 md:py-3 shadow-lg active:scale-[0.98] transition flex items-center justify-center gap-2 text-sm md:text-base">
                        <i class="fas fa-plus-circle"></i> 加入計畫
                    </button>
                </div>

                <!-- Task List -->
                <div class="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 no-scrollbar" id="task-list">
                    <!-- Tasks injected here -->
                </div>

                <!-- Empty State -->
                <div id="empty-state" class="hidden absolute inset-0 top-32 flex-col items-center justify-center text-gray-400 pointer-events-none">
                    <i class="fas fa-mug-hot text-5xl mb-3 text-indigo-100"></i>
                    <p class="text-sm">本日尚無計畫</p>
                </div>
            </div>

            <!-- VIEW: Monthly Calendar -->
            <div id="view-calendar" class="view-container absolute inset-0 flex flex-col bg-white transform translate-x-full overflow-y-auto z-20">
                
                <!-- Calendar Controls -->
                <div class="flex justify-between items-center p-4 shrink-0">
                    <button onclick="changeMonth(-1)" class="w-8 h-8 rounded-full hover:bg-gray-100 text-gray-600"><i class="fas fa-chevron-left"></i></button>
                    <h2 id="calendar-month-year" class="text-lg font-bold text-gray-800"></h2>
                    <button onclick="changeMonth(1)" class="w-8 h-8 rounded-full hover:bg-gray-100 text-gray-600"><i class="fas fa-chevron-right"></i></button>
                </div>

                <!-- Days Header -->
                <div class="grid grid-cols-7 text-center text-xs text-gray-400 font-medium mb-2 px-2 shrink-0">
                    <div>日</div><div>一</div><div>二</div><div>三</div><div>四</div><div>五</div><div>六</div>
                </div>

                <!-- Calendar Grid -->
                <div id="calendar-grid" class="grid grid-cols-7 gap-1 px-2 pb-4 shrink-0">
                    <!-- Days injected here -->
                </div>

                <!-- Mini List for Selected Day in Calendar -->
                <div class="flex-1 bg-gray-50 border-t border-gray-100 p-4 overflow-y-auto">
                    <div class="flex justify-between items-center mb-2">
                        <h3 class="text-sm font-bold text-gray-500">
                            <span id="preview-date-label">選擇日期以查看</span>
                        </h3>
                        <button onclick="editSelectedDate()" id="btn-edit-date" class="hidden text-xs bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full hover:bg-indigo-200 whitespace-nowrap">
                            去編輯 <i class="fas fa-arrow-right ml-1"></i>
                        </button>
                    </div>
                    <!-- 月曆下方的假日顯示 -->
                    <div id="calendar-holiday-info" class="hidden mb-2 text-xs text-red-500 font-bold bg-red-50 p-2 rounded-lg">
                        <i class="fas fa-star mr-1"></i> <span id="calendar-holiday-name"></span>
                    </div>
                    <ul id="calendar-preview-list" class="space-y-2 text-sm text-gray-600">
                        <li class="text-center py-4 text-gray-400">點擊上方日期來查看計畫</li>
                    </ul>
                </div>
            </div>

            <!-- VIEW: Shop (Redemption) -->
            <div id="view-shop" class="view-container absolute inset-0 flex flex-col bg-gray-50 transform translate-x-full z-20 overflow-hidden">
                <!-- Shop Header / Add Item -->
                <div class="p-3 md:p-4 bg-white border-b border-gray-100 shadow-sm z-10 shrink-0">
                    <h2 class="text-sm font-bold text-gray-500 mb-2">新增自訂獎勵</h2>
                    <div class="flex gap-2 mb-2">
                        <input type="text" id="shop-item-input" 
                            class="flex-[2] bg-gray-50 border border-gray-200 text-gray-900 rounded-lg md:rounded-xl focus:ring-2 focus:ring-purple-500 outline-none p-2 md:p-3 text-sm md:text-base" 
                            placeholder="來去買淘寶">
                        <input type="number" id="shop-cost-input" 
                            class="w-20 md:flex-[1] bg-gray-50 border border-gray-200 text-gray-900 rounded-lg md:rounded-xl focus:ring-2 focus:ring-purple-500 outline-none p-2 md:p-3 text-center text-sm md:text-base" 
                            placeholder="$">
                    </div>
                    <button onclick="addShopItem()" 
                        class="w-full bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg md:rounded-xl py-2 shadow-lg active:scale-[0.98] transition flex items-center justify-center gap-2 text-sm md:text-base">
                        <i class="fas fa-plus"></i> 新增獎勵
                    </button>
                </div>

                <!-- Shop List -->
                <div class="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 no-scrollbar">
                    <div id="shop-list" class="grid grid-cols-1 gap-3">
                        <!-- Shop Items injected here -->
                    </div>
                    
                    <div class="mt-6 mb-4">
                        <h3 class="text-sm font-bold text-gray-400 mb-2 px-1">兌換紀錄 (最近 5 筆)</h3>
                        <ul id="redemption-history" class="space-y-2 text-sm text-gray-500 px-1">
                            <!-- History injected here -->
                        </ul>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <script>
        // --- 設定與狀態 ---
        let tasks = JSON.parse(localStorage.getItem('myDailyPlans_v2')) || [];
        let shopItems = JSON.parse(localStorage.getItem('myShopItems')) || [
            { id: 1, text: '買一件衣服', cost: 50 },
        ];
        let redemptions = JSON.parse(localStorage.getItem('myRedemptions')) || [];

        if (!Array.isArray(tasks) || (tasks.length > 0 && !tasks[0].date)) {
            const oldTasks = JSON.parse(localStorage.getItem('myDailyPlans')) || [];
            if(oldTasks.length > 0) {
                const today = getTaipeiDateString();
                tasks = oldTasks.map(t => ({
                    id: Date.now() + Math.random(),
                    text: t.text,
                    reward: 0, completed: t.completed, date: today 
                }));
                localStorage.setItem('myDailyPlans_v2', JSON.stringify(tasks));
            }
        }

        let currentView = 'daily'; 
        let selectedDate = getTaipeiDateString(); 
        let calendarYear = new Date().getFullYear();
        let calendarMonth = new Date().getMonth(); 

        // --- 假日資料庫 (包含 2024, 2025, 2026) ---
        const holidays = {
            // 固定節日 (MM-DD)
            '01-01': '元旦',
            '02-28': '和平紀念日',
            '04-04': '兒童節', // 預設
            '04-05': '清明節', // 預設
            '05-01': '勞動節',
            '08-08': '父親節',
            '10-10': '國慶日',
            '10-31': '萬聖節',
            '12-25': '聖誕節',
            '12-31': '跨年',
            
            // --- 2025 台灣國定連假 ---
            '2025-01-25': '春節連假', '2025-01-26': '春節連假', '2025-01-27': '小年夜',
            '2025-01-28': '除夕', '2025-01-29': '春節 (初一)', '2025-01-30': '初二',
            '2025-01-31': '初三', '2025-02-01': '初四', '2025-02-02': '初五',
            '2025-02-28': '和平紀念日', '2025-03-01': '228連假', '2025-03-02': '228連假',
            '2025-04-03': '兒童節(補假)', '2025-04-04': '清明節', 
            '2025-04-05': '清明連假', '2025-04-06': '清明連假',
            '2025-05-11': '母親節',
            '2025-05-30': '端午節(補假)', '2025-05-31': '端午節', '2025-06-01': '端午連假',
            '2025-10-04': '中秋連假', '2025-10-05': '中秋連假', '2025-10-06': '中秋節',
            '2025-10-10': '國慶日', '2025-10-11': '國慶連假', '2025-10-12': '國慶連假',

            // --- 2026 預測/參考連假 (根據使用者提供) ---
            '2026-01-01': '元旦',

            // 2026 春節 (9天)
            '2026-02-14': '春節連假', '2026-02-15': '春節連假', '2026-02-16': '除夕',
            '2026-02-17': '春節 (初一)', '2026-02-18': '初二', '2026-02-19': '初三',
            '2026-02-20': '春節連假', '2026-02-21': '春節連假', '2026-02-22': '春節連假',

            // 2026 228 (3天)
            '2026-02-27': '228連假', '2026-02-28': '和平紀念日', '2026-03-01': '228連假',

            // 2026 兒童清明 (4天)
            '2026-04-03': '兒童節', '2026-04-04': '清明節',
            '2026-04-05': '清明連假', '2026-04-06': '清明連假',

            // 2026 勞動節 (3天)
            '2026-05-01': '勞動節', '2026-05-02': '勞動節連假', '2026-05-03': '勞動節連假',

            // 2026 端午 (3天)
            '2026-06-19': '端午連假', '2026-06-20': '端午節', '2026-06-21': '端午連假',

            // 2026 中秋 (4天)
            '2026-09-25': '中秋節', '2026-09-26': '中秋連假', 
            '2026-09-27': '中秋連假', '2026-09-28': '中秋連假',

            // 2026 國慶 (3天)
            '2026-10-09': '國慶連假', '2026-10-10': '國慶日', '2026-10-11': '國慶連假',

            // 2026 光復節 (3天 - 使用者提供)
            '2026-10-24': '光復節連假', '2026-10-25': '臺灣光復節', '2026-10-26': '光復節補假',

            // 2026 行憲紀念日 (3天 - 使用者提供)
            '2026-12-25': '行憲紀念日', '2026-12-26': '行憲紀念日連假', '2026-12-27': '行憲紀念日連假'
        };

        function getHolidayName(dateStr) {
            // 1. 檢查完整日期 (YYYY-MM-DD) - 優先權高
            if (holidays[dateStr]) return holidays[dateStr];
            
            // 2. 檢查固定日期 (MM-DD)
            const [y, m, d] = dateStr.split('-');
            const shortDate = `${m}-${d}`;
            // 如果這天在特定年份已經被定義過（例如清明節有時是 4/4 有時 4/5），則以 YYYY-MM-DD 為準
            if (holidays[shortDate]) {
                // 如果已經有特定的 YYYY-MM-DD 定義了這天，則不使用通用定義 (避免重複或錯誤覆蓋)
                // 但這裡簡化處理：只要沒在上面的 if 抓到，就會進到這裡，通常是安全的。
                return holidays[shortDate];
            }
            
            return null;
        }

        // --- 核心工具 ---
        function getTaipeiDateObject() {
            const now = new Date();
            const taipeiTime = new Date(now.toLocaleString("en-US", {timeZone: "Asia/Taipei"}));
            return taipeiTime;
        }

        function getTaipeiDateString(dateObj = null) {
            const d = dateObj || getTaipeiDateObject();
            const year = d.getFullYear();
            const month = String(d.getMonth() + 1).padStart(2, '0');
            const day = String(d.getDate()).padStart(2, '0');
            return `${year}-${month}-${day}`;
        }

        function formatDateDisplay(dateStr) {
            const [y, m, d] = dateStr.split('-');
            return `${m}月${d}日`;
        }

        function escapeHtml(text) {
            return text.replace(/[&<>"']/g, function(m) { return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]; });
        }

        // --- 金幣與Header邏輯 ---

        function calculateBalance() {
            const totalEarned = tasks
                .filter(t => t.completed)
                .reduce((sum, t) => sum + (parseInt(t.reward) || 0), 0);
            
            const totalSpent = redemptions
                .reduce((sum, r) => sum + (parseInt(r.cost) || 0), 0);
            
            return totalEarned - totalSpent;
        }

        function updateHeader() {
            const taipeiNow = getTaipeiDateObject();
            const todayStr = getTaipeiDateString(taipeiNow);
            const options = { month: 'long', day: 'numeric', weekday: 'long' };
            document.getElementById('header-date').textContent = new Intl.DateTimeFormat('zh-TW', options).format(taipeiNow);
            
            // Header 顯示今日節日
            const todayHoliday = getHolidayName(todayStr);
            const headerHolidayEl = document.getElementById('header-holiday');
            if (todayHoliday) {
                headerHolidayEl.textContent = todayHoliday;
                headerHolidayEl.classList.remove('hidden');
            } else {
                headerHolidayEl.classList.add('hidden');
            }

            const balance = calculateBalance();
            const balanceEl = document.getElementById('current-balance');
            balanceEl.textContent = balance;
            
            balanceEl.parentElement.classList.remove('text-yellow-300');
            void balanceEl.offsetWidth; 
            balanceEl.parentElement.classList.add('text-yellow-300');
        }

        function saveData() {
            localStorage.setItem('myDailyPlans_v2', JSON.stringify(tasks));
            localStorage.setItem('myShopItems', JSON.stringify(shopItems));
            localStorage.setItem('myRedemptions', JSON.stringify(redemptions));
            
            updateHeader();
            if (currentView === 'daily') renderTaskList();
            if (currentView === 'monthly') renderCalendar();
            if (currentView === 'shop') renderShop();
        }

        // --- 1. 列表視圖 (Daily) ---

        function renderTaskList() {
            const listEl = document.getElementById('task-list');
            const emptyState = document.getElementById('empty-state');
            const todayStr = getTaipeiDateString();
            
            const banner = document.getElementById('selected-date-banner');
            const editDateDisplay = document.getElementById('editing-date-display');
            
            // 處理「非今日」橫幅
            if (selectedDate !== todayStr) {
                banner.classList.remove('hidden');
                editDateDisplay.textContent = formatDateDisplay(selectedDate);
            } else {
                banner.classList.add('hidden');
            }

            // 處理「假日」橫幅
            const holidayName = getHolidayName(selectedDate);
            const holidayBanner = document.getElementById('holiday-alert-banner');
            const holidayNameSpan = document.getElementById('holiday-name');
            
            if (holidayName) {
                holidayBanner.classList.remove('hidden');
                let displayText = ` ${formatDateDisplay(selectedDate)} 是 ${holidayName}`;
                
                // 表情符號裝飾
                if (holidayName.includes('聖誕')) displayText = '🎄 ' + displayText;
                else if (holidayName.includes('年') || holidayName.includes('春節')) displayText = '🧧 ' + displayText;
                else if (holidayName.includes('端午')) displayText = '🛶 ' + displayText;
                else if (holidayName.includes('中秋')) displayText = '🥮 ' + displayText;
                else if (holidayName.includes('國慶')) displayText = '🇹🇼 ' + displayText;
                else if (holidayName.includes('母') || holidayName.includes('父')) displayText = '❤️ ' + displayText;
                else if (holidayName.includes('光復')) displayText = '🇹🇼 ' + displayText;
                else if (holidayName.includes('行憲')) displayText = '📜 ' + displayText;
                
                holidayNameSpan.textContent = displayText;
            } else {
                holidayBanner.classList.add('hidden');
            }

            const currentTasks = tasks.filter(t => t.date === selectedDate);
            
            listEl.innerHTML = '';
            
            if (currentTasks.length === 0) {
                emptyState.classList.remove('hidden');
                emptyState.classList.add('flex');
            } else {
                emptyState.classList.add('hidden');
                emptyState.classList.remove('flex');
                
                currentTasks.forEach(task => {
                    const li = document.createElement('div');
                    li.className = `bg-white p-3 rounded-lg md:rounded-xl shadow-sm border border-gray-100 flex items-center gap-2 md:gap-3 transition-all hover:shadow-md ${task.completed ? 'bg-gray-50' : ''}`;
                    li.innerHTML = `
                        <input type="checkbox" 
                            class="task-checkbox w-5 h-5 md:w-6 md:h-6 text-indigo-600 rounded-md md:rounded-lg border-gray-300 focus:ring-indigo-500 cursor-pointer accent-indigo-600 shrink-0"
                            ${task.completed ? 'checked' : ''} 
                            onchange="toggleTask(${task.id})">
                        
                        <div class="flex-1 min-w-0 transition-opacity ${task.completed ? 'opacity-50' : ''}">
                            <div class="text-gray-800 font-medium truncate text-sm md:text-base">${escapeHtml(task.text)}</div>
                            ${task.reward > 0 ? `<div class="text-xs text-yellow-600 font-bold mt-0.5"><i class="fas fa-coins"></i> 獎勵 $${task.reward}</div>` : ''}
                        </div>

                        <button onclick="deleteTask(${task.id})" class="w-8 h-8 flex items-center justify-center text-gray-300 hover:text-red-500 transition rounded-full hover:bg-red-50 shrink-0">
                            <i class="fas fa-trash-alt"></i>
                        </button>
                    `;
                    listEl.appendChild(li);
                });
            }
        }

        function addTask() {
            const input = document.getElementById('task-input');
            const rewardInput = document.getElementById('reward-input');
            const text = input.value.trim();
            const reward = parseInt(rewardInput.value) || 0;
            
            if (text) {
                tasks.push({
                    id: Date.now(),
                    text: text,
                    reward: reward,
                    completed: false,
                    date: selectedDate
                });
                input.value = '';
                rewardInput.value = '';
                saveData();
                input.focus();
            }
        }

        function toggleTask(id) {
            const task = tasks.find(t => t.id === id);
            if (task) {
                task.completed = !task.completed;
                saveData();
            }
        }

        function deleteTask(id) {
            if(confirm('確定要刪除嗎？')) {
                tasks = tasks.filter(t => t.id !== id);
                saveData();
            }
        }

        // --- 2. 月曆視圖 (Calendar) ---

        function renderCalendar() {
            const grid = document.getElementById('calendar-grid');
            const monthYearTitle = document.getElementById('calendar-month-year');
            
            grid.innerHTML = '';
            
            const dt = new Date(calendarYear, calendarMonth, 1);
            monthYearTitle.textContent = `${calendarYear}年 ${calendarMonth + 1}月`;
            
            const firstDayIndex = dt.getDay(); 
            const daysInMonth = new Date(calendarYear, calendarMonth + 1, 0).getDate();
            const todayStr = getTaipeiDateString();

            for (let i = 0; i < firstDayIndex; i++) {
                grid.appendChild(document.createElement('div'));
            }

            for (let d = 1; d <= daysInMonth; d++) {
                const dayStr = `${calendarYear}-${String(calendarMonth+1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
                const cell = document.createElement('div');
                const hasTask = tasks.some(t => t.date === dayStr);
                const isHoliday = getHolidayName(dayStr) !== null;
                
                let classes = 'calendar-day';
                if (dayStr === todayStr) classes += ' today';
                if (dayStr === selectedDate) classes += ' active';
                if (hasTask) classes += ' has-task';
                if (isHoliday) classes += ' is-holiday'; 

                cell.className = classes;
                cell.textContent = d;
                cell.onclick = () => selectDateInCalendar(dayStr);
                
                grid.appendChild(cell);
            }
            
            updateCalendarPreview();
        }

        function changeMonth(offset) {
            calendarMonth += offset;
            if (calendarMonth > 11) { calendarMonth = 0; calendarYear++; } 
            else if (calendarMonth < 0) { calendarMonth = 11; calendarYear--; }
            renderCalendar();
        }

        function selectDateInCalendar(dateStr) {
            selectedDate = dateStr;
            renderCalendar(); 
        }

        function updateCalendarPreview() {
            const previewList = document.getElementById('calendar-preview-list');
            const previewLabel = document.getElementById('preview-date-label');
            const editBtn = document.getElementById('btn-edit-date');
            
            // 假日資訊區塊
            const holidayInfo = document.getElementById('calendar-holiday-info');
            const holidayNameSpan = document.getElementById('calendar-holiday-name');

            const [y, m, d] = selectedDate.split('-');
            previewLabel.textContent = `${m}月${d}日 的計畫`;
            
            // 檢查假日
            const holidayName = getHolidayName(selectedDate);
            if (holidayName) {
                holidayInfo.classList.remove('hidden');
                holidayNameSpan.textContent = holidayName;
            } else {
                holidayInfo.classList.add('hidden');
            }

            const dayTasks = tasks.filter(t => t.date === selectedDate);
            
            previewList.innerHTML = '';
            if (dayTasks.length === 0) {
                previewList.innerHTML = '<li class="text-center py-2 text-gray-400">這天還沒有安排計畫</li>';
            } else {
                dayTasks.forEach(t => {
                    const li = document.createElement('li');
                    li.className = 'flex items-center gap-2 truncate';
                    li.innerHTML = `
                        <i class="fas fa-circle text-[6px] ${t.completed ? 'text-gray-300' : 'text-indigo-400'}"></i>
                        <span class="${t.completed ? 'line-through text-gray-400' : ''}">${escapeHtml(t.text)}</span>
                        ${t.reward > 0 ? `<span class="text-xs text-yellow-600 ml-auto">$${t.reward}</span>` : ''}
                    `;
                    previewList.appendChild(li);
                });
            }
            editBtn.classList.remove('hidden');
        }

        function editSelectedDate() {
            switchTab('daily');
        }

        // --- 3. 商店視圖 (Shop) ---

        function renderShop() {
            const shopListEl = document.getElementById('shop-list');
            const historyEl = document.getElementById('redemption-history');
            const balance = calculateBalance();

            shopListEl.innerHTML = '';
            
            if(shopItems.length === 0) {
                shopListEl.innerHTML = '<div class="text-center text-gray-400 py-4">還沒有獎勵商品，新增一個吧！</div>';
            } else {
                shopItems.forEach(item => {
                    const canAfford = balance >= item.cost;
                    const card = document.createElement('div');
                    card.className = `bg-white p-3 md:p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 ${!canAfford ? 'opacity-70' : ''}`;
                    card.innerHTML = `
                        <div class="flex items-center gap-3 w-full sm:w-auto">
                            <div class="w-10 h-10 rounded-full bg-purple-50 flex items-center justify-center text-purple-500 shrink-0">
                                <i class="fas fa-gift"></i>
                            </div>
                            <div class="min-w-0 flex-1">
                                <div class="font-medium text-gray-800 truncate text-sm md:text-base">${escapeHtml(item.text)}</div>
                                <div class="text-xs font-bold ${canAfford ? 'text-yellow-600' : 'text-red-400'}">
                                    <i class="fas fa-coins"></i> $${item.cost}
                                    ${!canAfford ? `<span class="font-normal text-red-300 ml-1">(差 $${item.cost - balance})</span>` : ''}
                                </div>
                            </div>
                        </div>
                        <div class="flex gap-2 w-full sm:w-auto justify-end border-t pt-2 sm:border-t-0 sm:pt-0 border-gray-100">
                             <button onclick="redeemItem(${item.id})" ${!canAfford ? 'disabled' : ''} 
                                class="flex-1 sm:flex-none px-4 py-1.5 rounded-lg text-sm font-bold text-white transition shadow-sm active:scale-95 ${canAfford ? 'bg-gradient-to-r from-yellow-400 to-orange-500 hover:shadow-md cursor-pointer' : 'bg-gray-300 cursor-not-allowed'}">
                                兌換
                            </button>
                            <button onclick="deleteShopItem(${item.id})" class="text-gray-300 hover:text-red-400 px-2">
                                <i class="fas fa-trash-alt text-sm"></i>
                            </button>
                        </div>
                    `;
                    shopListEl.appendChild(card);
                });
            }

            historyEl.innerHTML = '';
            const recentRedemptions = [...redemptions].reverse().slice(0, 5);
            if(recentRedemptions.length === 0) {
                historyEl.innerHTML = '<li class="text-center py-2 text-gray-300 text-xs">尚無兌換紀錄</li>';
            } else {
                recentRedemptions.forEach(r => {
                    const li = document.createElement('li');
                    li.className = 'flex justify-between items-center border-b border-gray-100 pb-1 last:border-0';
                    li.innerHTML = `
                        <span class="truncate pr-2">${escapeHtml(r.text)}</span>
                        <span class="text-xs text-gray-400 whitespace-nowrap">-${r.cost} <i class="fas fa-coins text-[10px]"></i></span>
                    `;
                    historyEl.appendChild(li);
                });
            }
        }

        function addShopItem() {
            const input = document.getElementById('shop-item-input');
            const costInput = document.getElementById('shop-cost-input');
            const text = input.value.trim();
            const cost = parseInt(costInput.value);

            if (text && cost > 0) {
                shopItems.push({
                    id: Date.now(),
                    text: text,
                    cost: cost
                });
                input.value = '';
                costInput.value = '';
                saveData();
            } else {
                alert('請輸入名稱與正確的金額');
            }
        }

        function deleteShopItem(id) {
            if(confirm('移除這個獎勵選項？')) {
                shopItems = shopItems.filter(item => item.id !== id);
                saveData();
            }
        }

        function redeemItem(id) {
            const item = shopItems.find(i => i.id === id);
            const balance = calculateBalance();
            
            if (item && balance >= item.cost) {
                if(confirm(`確定要花費 $${item.cost} 兌換「${item.text}」嗎？`)) {
                    redemptions.push({
                        id: Date.now(),
                        itemId: item.id,
                        text: item.text,
                        cost: item.cost,
                        date: new Date().toISOString()
                    });
                    saveData();
                    alert(`🎉 兌換成功！享受你的 ${item.text} 吧！`);
                }
            } else {
                alert('金幣不足！');
            }
        }

        function switchTab(tab) {
            currentView = tab;
            const viewList = document.getElementById('view-list');
            const viewCalendar = document.getElementById('view-calendar');
            const viewShop = document.getElementById('view-shop');
            
            const tabDaily = document.getElementById('tab-daily');
            const tabMonthly = document.getElementById('tab-monthly');
            const tabShop = document.getElementById('tab-shop');

            [viewList, viewCalendar, viewShop].forEach(el => {
                el.classList.add('translate-x-full');
                el.classList.remove('z-30'); 
            });
            [tabDaily, tabMonthly, tabShop].forEach(el => el.classList.remove('active'));

            if (tab === 'daily') {
                viewList.classList.remove('translate-x-full');
                tabDaily.classList.add('active');
                renderTaskList();
            } 
            else if (tab === 'monthly') {
                viewCalendar.classList.remove('translate-x-full');
                tabMonthly.classList.add('active');
                
                const [y, m] = selectedDate.split('-');
                calendarYear = parseInt(y);
                calendarMonth = parseInt(m) - 1;
                renderCalendar();
            }
            else if (tab === 'shop') {
                viewShop.classList.remove('translate-x-full');
                tabShop.classList.add('active');
                renderShop();
            }
            saveData(); 
        }

        document.getElementById('tab-daily').onclick = function() {
            if(currentView === 'daily') {
                selectedDate = getTaipeiDateString();
                renderTaskList();
            } else {
                switchTab('daily');
            }
        };

        function backToToday() {
            selectedDate = getTaipeiDateString();
            renderTaskList();
        }

        updateHeader();
        renderTaskList();
        setInterval(updateHeader, 60000);

    </script>
</body>
</html>
